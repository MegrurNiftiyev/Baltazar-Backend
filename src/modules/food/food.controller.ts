...
export const getFoodItemsController = catchAsync(async (req: Request, res: Response) => {
  ...
  const itemsWithLiked = attachIsLiked(result.items, req.user?.wishlist, 'FOOD');
  res.status(200).json({
    success: true,
...
export const getFoodItemByIdController = catchAsync(async (req: Request, res: Response) => {
  ...
  const itemWithLiked = attachIsLikedToItem(item, req.user?.wishlist, 'FOOD');
  res.status(200).json({ success: true, data: localize(itemWithLiked, req.lang!) });
});
