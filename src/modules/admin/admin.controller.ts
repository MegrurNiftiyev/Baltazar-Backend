...
export const reorderExploreSectionsController = catchAsync(async (req: Request, res: Response) => {
  const result = await adminService.reorderExploreSections(req.body);
  res.status(200).json({ success: true, data: result });
});
